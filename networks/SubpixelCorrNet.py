...
class SubpixelCorrNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return F.interpolate(out, size=(out.shape[2]*2, out.shape[3]*2), mode='bicubic'), \
                   F.interpolate(disps1, size=(disps1.shape[2]*2, out.shape[3]*2), mode='bicubic'), \
                   F.interpolate(disps2, size=(disps1.shape[2]*2, out.shape[3]*2), mode='bicubic')
        else:
            return F.interpolate(out, size=(out.shape[2]*2, out.shape[3]*2), mode='bicubic')
