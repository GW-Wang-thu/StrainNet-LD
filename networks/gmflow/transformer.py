...
class FeatureFlowAttention(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        # prob = torch.softmax(torch.matmul(query, self.k_proj(query).permute(0, 2, 1)) / (c ** 0.5), dim=-1)

        out = torch.matmul(torch.softmax(torch.matmul(query, self.k_proj(query).permute(0, 2, 1)) / (c ** 0.5), dim=-1), value)  # [B, H*W, 2]
    ...
    def forward_local_window_attn(self, feature0, flow,
        ...
        # feature0_window = F.unfold(self.k_proj(feature0.view(b, c, -1).permute(0, 2, 1)).permute(0, 2, 1).reshape(b, c, h, w),
        #                            kernel_size=kernel_size,
        #                            padding=local_window_radius).view(b, c, kernel_size ** 2, h, w).permute(
        #     0, 3, 4, 1, 2).reshape(b * h * w, c, kernel_size ** 2)  # [B, C*(2R+1)^2), H*W]
        ...
        #                        padding=local_window_radius).view(b, 2, kernel_size ** 2, h, w).permute(
        #     0, 3, 4, 2, 1).reshape(b * h * w, kernel_size ** 2, 2)  # [B, 2*(2R+1)^2), H*W]
        ...
        # prob = torch.softmax(torch.matmul(feature0_reshape, feature0_window) / (c ** 0.5), dim=-1)

        out = torch.matmul(torch.softmax(torch.matmul(self.q_proj(feature0.view(b, c, -1).permute(0, 2, 1)
                                                                  ).reshape(b * h * w, 1, c), F.unfold(
            self.k_proj(feature0.view(b, c, -1).permute(0, 2, 1)).permute(0, 2, 1).reshape(b, c, h, w),
            kernel_size=kernel_size,
            padding=local_window_radius).view(b, c, kernel_size ** 2, h, w).permute(
            0, 3, 4, 1, 2).reshape(b * h * w, c, kernel_size ** 2)) / (c ** 0.5), dim=-1),
                           F.unfold(flow, kernel_size=kernel_size,
                                    padding=local_window_radius).view(b, 2, kernel_size ** 2, h, w).permute(
                               0, 3, 4, 2, 1).reshape(b * h * w, kernel_size ** 2, 2)).view(b, h, w, 2).permute(0, 3, 1, 2).contiguous()  # [B, 2, H, W]
